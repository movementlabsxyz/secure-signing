...
use secure_signer::cryptography::ToBytes;
use secure_signer::{cryptography::Curve, Signer, Signing};
...
impl AppState {
	pub fn new() -> Self {
		Self { public_key: Mutex::new(None) }
	}
}

pub fn create_server<O, C>(hsm: Arc<Mutex<Signer<O, C>>>, app_state: Arc<AppState>) -> Router
where
	...
	C: Curve,
{
	...
	let public_key = hsm.lock().await.public_key().await.map_err(|e| {
	...
	Ok(Json(SignedResponse { signature: signature.to_bytes(), public_key: public_key.to_bytes() }))
}
...
async fn verify_ed25519(payload: &VerifyRequest) -> Result<Json<VerifyResponse>, StatusCode> {
	...
	let public_key_bytes: &[u8; 32] = payload.public_key.as_slice().try_into().map_err(|_| {
	...
	let signature_bytes: &[u8; 64] = payload.signature.as_slice().try_into().map_err(|_| {
