mod ed25519 {
	use secure_signer::cryptography::ed25519::{Ed25519, PublicKey};
	use secure_signer::{Signing, Verify};
	use secure_signer_test::ed25519::TestSigner;
...
mod secp256k1 {
	...
	use secure_signer::Verify;
	use secure_signer_aws_kms::hsm::AwsKms;
	use secure_signer_local::signer::LocalSigner;
	...
	async fn local_signing_verify() -> Result<(), anyhow::Error> {
		...
		assert!(secure_signer::cryptography::secp256k1::Secp256k1::verify(
			&digest,
			&signature,
			&public_key
		)
