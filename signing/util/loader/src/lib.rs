...
use secure_signer::key::SignerBuilder;
use secure_signer::{
	...
	C: Curve,
{
	async fn sign(
		&self,
		message: &[u8],
	) -> Result<<C as Curve>::Signature, secure_signer::SignerError> {
		debug!("using a loaded signer to sign a message");
		self.signer.sign(message).await
	}

	async fn public_key(&self) -> Result<<C as Curve>::PublicKey, secure_signer::SignerError> {
	...
	async fn load(&self) -> Result<LoadedSigner<Secp256k1>, LoaderError> {
		...
		match self {
			SignerIdentifier::Local(local) => {
				let signer = secure_signer_local::signer::LocalSigner::from_signing_key_hex(
			...
			SignerIdentifier::AwsKms(aws_kms) => {
				let builder =
					secure_signer_aws_kms::hsm::key::Builder::new().create_key(aws_kms.create);
...
impl Load<Ed25519> for SignerIdentifier {
	async fn load(&self) -> Result<LoadedSigner<Ed25519>, LoaderError> {
		...
		match self {
			SignerIdentifier::Local(local) => {
				let signer = secure_signer_local::signer::NoSpecLocalSigner::<
					secure_signer_local::signer::ed25519::Ed25519SignerInner,
					Ed25519,
			...
			SignerIdentifier::HashiCorpVault(hashi_corp_vault) => {
				let builder = secure_signer_hashicorp_vault::hsm::key::Builder::new()
